...
app.get('/points/:id', async (req, res,) => {
	res.json(await db.Point.findByPk(+req.params.id));
});
...
(async function () {
	...
	db.Point.create({name: 'Minsk', time: 60*10*10, vehicle: 'bike'});
	db.Point.create({name: 'Molodechno', time: 60*20*10, vehicle: 'car'});
	db.Point.create({name: 'Klichau', time: 60*60*10, vehicle: 'walk'});
